...
/**
 * Icon paths are copied verbatim from the provided SVG asset pack
 * (public/svg/*.svg) and reproduced here as inline React components so
 * that stroke/fill can theme via currentColor instead of being baked
 * into static raster files.
 */
const PATHS = {
  "arrow-path": (
    <path
...
export function Icon({ name, size = 20, ...props }: IconProps) {
  return (
    <svg
      ...
      width={size}
      height={size}
      viewBox={name === "cube" ? "0 0 16 16" : name === "search" ? "0 0 20 20" : "0 0 24 24"}
      aria-hidden="true"
      focusable="false"
      {...props}
    >
